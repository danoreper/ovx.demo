## A helper script which is kicked off to execute each individual job that is parallelized. 
## THis script loads up the function and inputs, uses do.call, and then writes it out to a file
## Code downstream of this needs to wait for the parallelization code to finish blocking, and
## then load up each individual file to get the individual jobs.
##


## func, funcArgs, and outFile must be defined in the session

source("./utils/loadParamsFunc.R")
